...
from Marksheet_Backend import retrieve
def marksheet():
       root = Tk()
       ...
       def Add():
              try:
                     ...
                     validate_email(em)
                     if (len(roll.get()) != 0):
                            Marksheet_Backend.insert(name.get(), roll.get(), fname.get(), mname.get(), DOB.get(),
                                                     gender.get(), \
                                                     scl.get(), email.get(), m1.get(), m2.get(), m3.get(), m4.get(),
                                                     m5.get(), \
                                                     gt.get(), per.get(), cgpa.get(), grade.get(), div.get(),
                                                     result.get())
              except EmailNotValidError as e:
                     showerror(title="validation error",message="Email not Valid")
       ...
       def retriev():
              for row in Marksheet_Backend.retrieve(stdid=roll.get()):
                     print(row)
                     Entry_Name.delete(0, END)
                     Entry_Name.insert(END, row[0])
                     Entry_Father_Name.delete(0, END)
                     Entry_Father_Name.insert(END, row[1])
                     Entry_Mother_Name.delete(0, END)
                     Entry_Mother_Name.insert(END, row[2])
                     Entry_DOB.delete(0,END)
                     Entry_DOB.insert(END,row[3])
                     Entry_Gender.delete(0,END)
                     Entry_Gender.insert(END,row[4])
       ...
       def Compute():
              ...
              if Per <= 45 and result != "FAIL":
                     ...
                     div.set('FIRST')
       def printme():
              createreceipt.printmarksheet(std_name=name.get(),mothername=mname.get(),depart="IT",emv=m1.get(),python=m2.get(),COA=m3.get(),OS=m4.get(),CNND=m5.get(),GT=gt.get(),BAtch=div.get(),CGPA=cgpa.get(),Result=result.get())
       def Reset():
              name.set(' ')
              roll.set(' ')
              fname.set(' ')
       ...
       Label_Roll_no = Label(Frame_1, text = 'Student Id', font = ('arial',15,'bold'), bg = 'Navajo white')
       ...
       Entry_DOB = DateEntry(Frame_1, selectmode='day', maxdate=dt1,width = 40,height=50)
       ...
       ret_btn = Button(Frame_1,text = 'Retrieve', font = ('arial',15,'bold'),command=retriev)
       ret_btn.grid(row = 3, column = 1, padx = 5, pady = 5)
       ...
       Label_10 = Label(Frame_2, text = 'GRADING', font = ('arial',16,'bold'), bg = 'Navajo white')
       ...
       Button_print =  Button(Frame_2, text = 'Print Result', font = ('arial',16,'bold'), bg = 'white',command=printme)
       Button_print.grid(row = 11, column = 4)
...
def search_result_marksheet(row):
       ...
       iname = StringVar(Frame_1,value=row[0][1])
       iroll = StringVar(Frame_1,value=row[0][2])
       ifname = StringVar(Frame_1,value=row[0][3])
       imname = StringVar(Frame_1,value=row[0][4])
       iDOB = StringVar(Frame_1,value=row[0][5])
       igender = StringVar(Frame_1,value=row[0][6])
       ...
       Label_Name = Label(Frame_1, text = 'Name', font = ('arial',15,'bold'), bg = 'Navajo white')
       Label_Name.grid(row = 0, column = 0, padx = 80)
       Entry_Name = Entry(Frame_1, font = ('arial',15), width = 25, textvariable = iname)
       Entry_Name.grid(row = 0, column = 1, padx = 5, pady = 5)

       Label_Roll_no = Label(Frame_1, text = 'Roll Number', font = ('arial',15,'bold'), bg = 'Navajo white')
       Label_Roll_no.grid(row = 0, column = 3, padx = 80)
       Entry_Roll_no = Entry(Frame_1, font = ('arial',15), width = 25, textvariable = iroll)
       Entry_Roll_no.grid(row = 0, column = 4, padx = 40)

       Label_Father_Name = Label(Frame_1, text = 'Father Name', font = ('arial',15,'bold'), bg = 'Navajo white')
       Label_Father_Name.grid(row = 1, column = 0, padx = 80)
       Entry_Father_Name = Entry(Frame_1, font = ('arial',15), width = 25, textvariable = ifname)
       Entry_Father_Name.grid(row = 1, column = 1, padx = 5, pady = 10)

       Label_Mother_Name = Label(Frame_1, text = 'Mother Name', font = ('arial',15,'bold'), bg = 'Navajo white')
       Label_Mother_Name.grid(row = 1, column = 3, padx = 80)
       Entry_Mother_Name = Entry(Frame_1, font = ('arial',15), width = 25, textvariable = imname)
       Entry_Mother_Name.grid(row = 1, column = 4, padx = 5)

       Label_DOB = Label(Frame_1, text = 'Date of Birth', font = ('arial',15,'bold'), bg = 'Navajo white')
       Label_DOB.grid(row = 2, column = 0, padx = 80)
       Entry_DOB = DateEntry(Frame_1, selectmode='day', year=2020, month=5, day=22,
                                       font=('arial', 17, 'bold'))
       Entry_DOB.grid(row = 2, column = 1, padx = 5, pady = 5)

       Label_Gender = Label(Frame_1, text = 'Gender', font = ('arial',15,'bold'), bg = 'Navajo white')
       Label_Gender.grid(row = 2, column = 3, padx = 80)
       Entry_Gender = Entry(Frame_1, font = ('arial',15), width = 25, textvariable = igender)
       Entry_Gender.grid(row = 2, column = 4, padx = 5, pady = 5)
       ...
       Btn_Exit.grid(row = 8, column = 4, padx = 50, pady = 6)
       Btn_print = Button(Frame_2, text='PRINT', font=('arial', 12, 'bold'), width=10, command=root.destroy)
       Btn_print.grid(row=9, column=4, padx=50, pady=6)
