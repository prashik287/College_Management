...
class Library:
       ...
       #================================================Functions======================================================
              def BookRec(event):
                     try:
                            ...
                            if index:
                                   ...
                                   selected_tuple = self.table.selection()[0]
              ...
              def Insert():
                     s = gen_ref()
                     self.Entry_1.insert(END,s)
                     self.Entry_1.configure(state=tkinter.DISABLED)
                     ...
                     if(len(self.refno.get()) != 0):
                            ...
                            self.table.insert(END , (self.Mtype.get(), self.refno.get(), self.fname.get(), self.surname.get()\
                                                         , self.address.get(), self.post.get(), self.mobno.get(), self.ID.get()\
                                                         , self.title.get(), self.author.get(), self.borrow.get(), self.due.get()\
                                                         , self.loan.get()))
              ...
              def Delete():
                     ...
                     # Library_Backend.delete(selected_tuple)
                     Reset()
                     Display()
              ...
              def insertb():
                     for row in self.table.get_children():
                            ...
                            self.Entry_0.delete(0, END)
                            self.Entry_0.insert(END, self.table.item(row)['values'][1])
                            self.Entry_1.delete(0, END)
                            self.Entry_1.insert(END, self.table.item(row)['values'][0])
                            # print(self.table.item(row)['values'][1])
                            self.Entry_2.delete(0, END)
                            self.Entry_2.insert(END, self.table.item(row)['values'][2])
                            self.Entry_3.delete(0, END)
                            self.Entry_3.insert(END, self.table.item(row)['values'][3])
                            self.Entry_4.delete(0, END)
                            self.Entry_4.insert(END, self.table.item(row)['values'][4])
                            self.Entry_5.delete(0, END)
                            self.Entry_5.insert(END, self.table.item(row)['values'][5])
                            self.Entry_6.delete(0, END)
                            self.Entry_6.insert(END, self.table.item(row)['values'][6])
                            self.Entry_7.delete(0, END)
                            self.Entry_7.insert(END, self.table.item(row)['values'][7])
                            self.Entry_8.delete(0, END)
                            self.Entry_8.insert(END, self.table.item(row)['values'][8])
                            self.Entry_9.delete(0, END)
                            self.Entry_9.insert(END, self.table.item(row)['values'][9])
                            self.Entry_10.delete(0, END)
                            self.Entry_10.insert(END, self.table.item(row)['values'][10])
                            self.Entry_11.delete(0, END)
                            self.Entry_11.insert(END, self.table.item(row)['values'][11])
              def Search():
                     global a
                     for row in self.table.get_children():
                            self.table.delete(row)
                     try:
                            for row in Library_Backend.search(self.Entry_1.get()):
                                   # print(row)
                                   self.table.insert(parent='', index=0, values=row)
                                   insertb()
              ...
              def gen_ref():
                     a = random.randint(111111, 999999)
                     print(a)
                     # self.Entry_1.delete(0, tkinter.END)  # Clear existing content
                     # self.Entry_1.insert(tkinter.END, a)
                     return  a
       ...
       #===========================================List Box and Scroll Bar==========================================                    
              ...
              sb_2.grid(row = 0, column = 1, sticky = 'ns')
              #
              self.table = Treeview(Detail_Frame, columns=(
                      'mtype','refno' ,'First_Name', 'Last_name', 'Address', 'postcode', 'mobile', 'bookid',
                     'Boot_title', 'Author',
                     'Borrowed_date', 'Due_date'), show=('headings'))

              self.table.heading('mtype', text='Member Type')
              ...
              self.table.heading('Boot_title', text='Boot_title')
              self.table.heading('Author', text='Author')
              self.table.heading('Borrowed_date', text='Borrowed_date')
              self.table.heading('Due_date', text='Due_date')
              # self.table.heading('Loan_days', text='Loan_days')
              self.table.grid(row=3, column=0, sticky='ns')
              self.table.column('mtype', width=80)
              self.table.column('refno', width=80)
              self.table.column('First_Name', width=80)
              self.table.column('Last_name', width=80)
              self.table.column('Address', width=100)
              self.table.column('postcode', width=80)
              self.table.column('bookid', width=80)
              self.table.column('Boot_title', width=80)
              self.table.column('Author', width=80)
              self.table.column('Borrowed_date', width=80)
              self.table.column('Due_date', width=70)
              # self.table.column('Loan_days', width=80)
