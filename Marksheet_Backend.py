...
def connect():
       ...
       cur.execute('CREATE TABLE IF NOT EXISTS Marks (id INTEGER PRIMARY KEY, name text, stdid text, fname text, mname \
                     ...
                     foreign key(stdid) references "student" on delete cascade)')
...
def search(roll):
       ...
       cur.execute('SELECT * FROM Marks WHERE stdid = ?',(roll,))
       row = cur.fetchall()
       return row
...
def retrieve(stdid=""):
       con = sqlite3.connect('college.db')
       cur = con.cursor()
       cur.execute("Select name,fname,mname,dob,gender from student where stdid = ?",(stdid,))
       a=cur.fetchall()
       return  a
connect()
